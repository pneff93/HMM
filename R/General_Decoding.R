...
#' 
#' @description Calculation of local and global decoding given the dataset and the 
#' coresponding parameters of the HMM
#' 
#' 
#' 
...
decode <- function(x, m, L1, L2, L3=NULL, L4=NULL, L5=NULL, gamma,sigma,theta,multi=FALSE){
  ...
  #computing the alpha and beta matrices and the Likelihood L_T that is the sum of the weigths 
  N <- length (x)
  ...
  #individually via the simple scalar multiplication
  probmatrix <-matrix(,nrow=N,ncol=m)
  for (i in 1:m){
  probmatrix[,i] <- alpha[,i]*beta[i,]/L_T
  }
  
  #for each row we now want the most likely state as return (which is the column name)
  local_path_out <- apply(probmatrix,1,which.max)
  
  ##########################################################################
  #Global decoding
  
  #Searching for the most likely branch 
  ...
  #t = 2,...,N
  #again with scalar multiplication we get by (etha[i,]*gamma)
  #the corresponding matrix and take the max value of each 
  #collumn, thus selecting the max (i) for each state j
  #then we multiply each max with the corresponding probability
  for ( i in 2:N){
    w <- apply (etha[(i-1),]*gamma,2,max)*p[set+i-1]
    etha[i,] <- w/sum(w)
  }
  
  #after computing the etha we now have to reconstruct the optimal path recursevly:
  # with i_T = argmax etha_Ti
  #and i_t =argmax (etha_ti * gamma_i,i+1)
